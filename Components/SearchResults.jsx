        ...
        <div className="h-screen m-4 overflow-y-auto">
            {
                props.searchedStock.map((stock)=>(
                    <div className="flex flex-row gap-12 border-[1px] p-2 rounded-lg border-gray-800 m-2" key={stock.token}>
                        ...
                        <span className="text-lg font-semibold text-white uppercase basis-1/2">{stock.symbol}</span>
                        <button onClick={()=>{addStock(stock.token)}} className="w-10 h-10 ml-auto text-2xl text-green-500 border-2 border-green-500">+</button>
                    </div>
                ))
            }
        </div>                
